...
import { UtensilsCrossed, Footprints, ShowerHead, Sofa, Wrench, WashingMachine, Grid2X2, Layers, TreePine, FlipHorizontal2, Gem } from 'lucide-react';
...
const MaintenanceGuide = () => {
  ...
  const frequencies = {
    daily: {
      ...
      sections: [
        { area: "Kitchen", task: "Wipe counters immediately after cooking. Dry sink area.", icon: <UtensilsCrossed size={28} color="#c9a96e" /> },
        { area: "Floors", task: "Sweep/Vacuum daily. Mop with mild cleaner avoiding excess water.", icon: <Footprints size={28} color="#c9a96e" /> },
        { area: "Bathrooms", task: "Squeegee glass partitions. Ensure exhaust fans run.", icon: <ShowerHead size={28} color="#c9a96e" /> },
        { area: "Furniture", task: "Dust surfaces with a microfiber cloth. Use coasters.", icon: <Sofa size={28} color="#c9a96e" /> }
      ]
    },
    monthly: {
      ...
      sections: [
        { area: "Hardware", task: "Tighten loose hinges/handles. Check drawer channels.", icon: <Wrench size={28} color="#c9a96e" /> },
        { area: "Appliances", task: "Clean chimney filters. Descale washing machine/dishwasher.", icon: <WashingMachine size={28} color="#c9a96e" /> },
        { area: "Upholstery", task: "Vacuum sofas and mattresses. rotate cushions.", icon: <Sofa size={28} color="#c9a96e" /> },
        { area: "Grout", task: "Check tile grout for discoloration or cracks.", icon: <Grid2X2 size={28} color="#c9a96e" /> }
      ]
    }
  };
  ...
  const materials = [
    {
      ...
      icon: <Layers size={32} color="#c9a96e" />,
    ...
    {
      name: "Glass & Mirrors",
      icon: <FlipHorizontal2 size={32} color="#c9a96e" />,
      dos: ["Glass cleaner liquid", "Newspaper for streak-free", "Circular wiping motion"],
      donts: ["Abrasive sponges", "Acidic cleaners", "Excessive pressure"]
    },
    {
      name: "Quartz/Stone",
      icon: <Gem size={32} color="#c9a96e" />,
  ...
  return (
    <div className="blog-page">
      ...
      <div className="blog-body">
        ...
        <section style={{ marginBottom: '6rem' }}>
          <h2 className="blog-section-title">Care <em>Schedule</em></h2>
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
            <button onClick={() => setFrequency('daily')} style={{
              ...
              color: frequency === 'daily' ? '#fff' : 'var(--kr-dark)', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.1em'
            }}>Daily & Weekly</button>
            <button onClick={() => setFrequency('monthly')} style={{
              padding: '1rem 2rem',
              background: frequency === 'monthly' ? 'var(--kr-red)' : 'transparent',
              border: '1px solid var(--kr-red)',
              color: frequency === 'monthly' ? '#fff' : 'var(--kr-dark)', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.1em'
            }}>Monthly</button>
          </div>
            ...
            <div className="blog-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem' }}>
              {frequencies[frequency].sections.map((sec, i) => (
                <div key={i} style={{ padding: '1.5rem', background: 'rgba(255,255,255,0.03)' }}>
                  ...
                  <h4 style={{ marginBottom: '0.5rem' }}>{sec.area}</h4>
        ...
        <section style={{ marginBottom: '6rem' }}>
          <h2 className="blog-section-title">Material <em>Specifics</em></h2>
